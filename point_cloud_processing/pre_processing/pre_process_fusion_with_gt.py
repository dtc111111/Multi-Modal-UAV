...
figure_folder = "/media/yi/KESU/anti_uav/figure/png/fusion_plot_with_gt_val"
...
for seq_folder in os.listdir(dataset_folder):
    ...
    if accumulated_data.shape[0] < 5e4:
        ...
        ax.scatter(accumulated_gt[:,0], accumulated_gt[:,1], accumulated_gt[:,2], s=1, c = 'r')
        ax = fig.add_subplot(132, projection = '3d')
        ax.scatter(detections[:,1], detections[:,2], detections[:,3], s=1, c = 'b')
        ax.scatter(accumulated_gt[:,0], accumulated_gt[:,1], accumulated_gt[:,2], s=1, c = 'r')
